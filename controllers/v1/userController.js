    ...
    if (isMatched) {
        ...
        let accessToken = await generateJWTToken(user._id), criteria = {_id: user._id},
            ...
            options = {
                ...Options,
                new: true,
                projection: {password: NOT, tokenManager: NOT, __v: NOT, signUpType: NOT}
            };

        updatedData = await updateData(userModel, criteria, dataToUpdate, options);
        return RESPONSEMESSAGES.SUCCESS.MISSCELANEOUSAPI(MESSAGES.LOGGED_IN_SUCCESSFULLY, {
            ...updatedData,
            accessToken
        });
...
userController.forgotPassword = async (body) => {
    return new Promise(async (resolve, reject) => {
        ...
        let OTP = generateOTP(), currentDate = new Date(),
            expiryTime = await addTimeToDate(currentDate, OTP_EXPIRY.TIME_TO_ADD, OTP_EXPIRY.PREFIX);
        let dataToUpdate = {
            $set: {
                OTP: OTP,
                createdAt: new Date(),
